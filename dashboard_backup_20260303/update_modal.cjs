...
if (startSearchIdx === -1) {
    ...
    console.log('Context around returnPagesIdx:', content.substring(returnPagesIdx, returnPagesIdx + 100)); // Debug
    process.exit(1);
}
...
if (replaceStartIndex >= replaceEndIndex) {
    ...
    console.log('StartIndex:', replaceStartIndex, 'EndIndex:', replaceEndIndex);
    process.exit(1);
}
...
window.__openProduct = async function (product) {
    ...
    <div id="tab-ai" class="modal-tab-content">
      <div class="ai-summary">
        ...
        <p style="font-size:11px; color:var(--text-muted); margin-top:16px; border-top:1px solid var(--border); padding-top:12px;">
          * 이 요약은 \${formatNumber(product.review_count || 0)}개의 리뷰를 바탕으로 AI가 생성한 가상의 분석 결과입니다. (프로토타입)
        </p>
      </div>
    </div>
  ...
  window.switchTab = async (tabName) => {
    ...
    // Find button by text content or index? Easy way: re-query by onclick attribute or add id
    // Ideally adding IDs to buttons would be cleaner, but let's just find by onclick
    // We can rely on the fact that buttons are in order: Info, Rank, AI
    ...
    if (tabName === 'rank') {
      await loadRankChart(product.product_id); // Ensure loadRankChart is defined in scope or globally
    }
  };
};
...
async function loadRankChart(productId) {
  ...
  try {
    // We assume fetchRankHistory is available globally or imported. 
    // Since main.js is a module, we should be careful. 
    // But fetchRankHistory is imported at the top of main.js in the original file.
    // We are only replacing the middle part, so imports remain.
